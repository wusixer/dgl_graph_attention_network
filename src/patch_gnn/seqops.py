...
def one_hot(df: pd.DataFrame, padding_length: int) -> np.ndarray:
    ...
    if padding_length < in_max_len:
        print(
            f"the input padding_length should be longer or equal to the max length of the inputs, resetting padding_length from {padding_length}  to max sequence length {in_max_len}"
        )
